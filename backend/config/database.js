...
const config = {
 ...
 const connection = mysql.createConnection(config);
  ...
  connection.connect(function(err) {
    ...
    let createRequests = `create table if not exists requests(id int AUTO_INCREMENT, managerID INT, employeeID INT, startDate DATE, endDate DATE, numOfDays INT,type VARCHAR(50), status VARCHAR(25), rejectionReason VARCHAR(500), createdAt TIMESTAMP, PRIMARY KEY(id), CONSTRAINT FK_employeeID FOREIGN KEY (employeeID) REFERENCES employees(id), CONSTRAINT FK_managerID FOREIGN KEY (managerID) REFERENCES employees(id))`;
